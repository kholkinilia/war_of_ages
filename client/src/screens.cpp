...
void update_widgets(tgui::Gui &gui, const client_state &state) {
    switch (state.get_cur_screen()) {
        ...
        case screen::GAME_SCREEN: {
            if(state.get_cur_game_state() == nullptr) return;
            ...
            for (auto unit : p1.units) {
                unit_picture = tgui::Picture::create(*(sprite_supplier::get_instance().
                        get_unit_sprite(unit.type(), sprite_supplier::player_side::LEFT).getTexture()));
                unit_picture->setPosition(unit.position(), 100);
                units_group->add(unit_picture);
            }
            for (auto unit : p2.units) {
                unit_picture = tgui::Picture::create(*(sprite_supplier::get_instance().
                    get_unit_sprite(unit.type(), sprite_supplier::player_side::RIGHT).getTexture()));
                unit_picture->setPosition(unit.position(), 100);
                units_group->add(unit_picture);
            }
        }

...
state bot_actions_receiver::get_state(const std::pair<player_snapshot, player_snapshot> &p, int player) {
    ...
    answer.damage.first = std::min(answer.damage.first / 5, static_cast<int>(Q_table.size()) - 1);
    answer.damage.second = std::min(answer.damage.second / 5, static_cast<int>(Q_table.size()) - 1);
...
static double get_max(std::vector<float> v) {
...
bot_actions_receiver::action bot_actions_receiver::get_action(state state) {
    ...
    std::for_each(
        ...
        Q_table[state.damage.first][state.damage.second].begin(),
        Q_table[state.damage.first][state.damage.second].end(), [&delta](float &d) { d -= delta; });

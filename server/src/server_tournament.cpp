...
void war_of_ages::server::server_tournament::match_participants_lock_held() {
    ...
    for (std::size_t i = 0; i < m_participants.size(); i++) {
        ...
        order[i].second = i;
        std::cerr << i << ": " << m_is_playing[i] << " " << m_participants[i] << " " << games_played[i]
                  << std::endl;
        std::cerr << std::count(m_match_results[i].begin(), m_match_results[i].end(), game_result::NONE) << std::endl;
        std::cerr << "size: " << m_match_results[i].size() << std::endl;
        for (std::size_t j = 0; j < m_match_results[i].size(); j++) {
